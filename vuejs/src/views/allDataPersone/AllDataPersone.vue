<template>
  <div>
    <div class="all_data_table">
      ...
      <div v-for="(item, index) in isResult" :key="index" class="pt-5">
        ...
        >
          ...
          <div class="data_table_block" v-if="val">
            <div
              v-if="name === 'first_passport_page'"
              class="form_block_title d-block"
            >
              {{ val[0].name }}
            </div>
              ...
              <div v-for="(val2, name2) in val" :key="name2">
                <div v-if="isObject(val2)">
                  <div v-for="(val3, key) in item" :key="key">
                    <div v-if="val3">{{ isTitle(key) }} => {{ val3 }}</div>
                  </div>
                </div>
    ...
    <v-btn elevation="2" class="card_content_button" large @click="del()"
      >Удалить</v-btn
    >
    <v-btn elevation="2" class="card_content_button" large @click="add()"
      >Добавить</v-btn
    >
    <v-btn elevation="2" class="card_content_button" large @click="next()"
      >Продолжить</v-btn
    >
  </div>
</template>
  ...
  methods: {
    ...
    isTitle(element) {
      switch (element) {
        ...
        case "foreigner_doc_number":
          return "Номер документа удостоверяющего личность";
        case "foreigner_doc_issued":
          return "Дата начала срока действия права пребывания";
        case "foreigner_doc_validity":
          return "Дата окончания срока действия пребывания";
        case "supervisory":
          return "Наименования наблюдательного совета";
        case "first_passport_page":
          return "Паспорт";
        default:
          return element;
      }
